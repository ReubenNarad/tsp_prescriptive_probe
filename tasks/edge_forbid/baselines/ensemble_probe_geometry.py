...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Ensemble an edge-forbid probe with a geometry-only set-transformer baseline. "
            "Alpha chosen on val top-1."
        )
    )
    p.add_argument("--probe_reps_path", type=str, required=True, help="probe_reps.pt (edge-forbid)")
    p.add_argument("--probe_model_path", type=str, required=True, help="Saved probe model .pt (from train_probes.py --save_models)")
    p.add_argument(
        "--baseline_reps_path",
        type=str,
        required=True,
        help="probe_reps_edge_geometry.pt (or other geometry-only reps)",
    )
    p.add_argument(
        "--baseline_model_path",
        type=str,
        required=True,
        help="Saved geometry-only model .pt (from train_probes.py --save_models)",
    )
...
def main() -> None:
    ...
    probe_reps_path = Path(args.probe_reps_path).expanduser().resolve()
    probe_model_path = Path(args.probe_model_path).expanduser().resolve()
    baseline_reps_path = Path(args.baseline_reps_path).expanduser().resolve()
    baseline_model_path = Path(args.baseline_model_path).expanduser().resolve()
    out_path = Path(args.out_path).expanduser().resolve()

    device = _as_device(args.device)
    probe_logits, y_true, valid, instance_ids, node_ids, B, n = _load_probe_logits(
        reps_path=probe_reps_path,
        model_path=probe_model_path,
        device=device,
        batch_size_instances=int(args.batch_size_instances),
    )
    base_logits, y_true_b, valid_b, instance_ids_b, node_ids_b, B_b, n_b = _load_probe_logits(
        reps_path=baseline_reps_path,
    ...
    if B != B_b or n != n_b:
        raise ValueError("Probe/baseline reps have different instance counts")
    if not (np.array_equal(instance_ids, instance_ids_b) and np.array_equal(node_ids, node_ids_b)):
        raise ValueError("Probe/baseline ordering mismatch after sort")
    if not (np.array_equal(valid, valid_b) and np.array_equal(y_true, y_true_b)):
        raise ValueError("Probe/baseline labels or valid mask mismatch")

    if args.score_norm == "zscore":
        probe_logits = _zscore_by_instance(probe_logits, instance_ids, valid)
        base_logits = _zscore_by_instance(base_logits, instance_ids, valid)
    ...
    for alpha in alpha_grid:
        mix = alpha * probe_logits + (1.0 - alpha) * base_logits
    ...
    mix = best_alpha * probe_logits + (1.0 - best_alpha) * base_logits

    out = {
        "config": {
            "probe_reps_path": str(probe_reps_path),
            "probe_model_path": str(probe_model_path),
            "baseline_reps_path": str(baseline_reps_path),
        ...
        "probe_only": {
            "train": _eval_scores(probe_logits, y_true, valid, instance_ids, train_mask),
            "val": _eval_scores(probe_logits, y_true, valid, instance_ids, val_mask),
            "test": _eval_scores(probe_logits, y_true, valid, instance_ids, test_mask),
        },
        "baseline_only": {
            "train": _eval_scores(base_logits, y_true, valid, instance_ids, train_mask),
            "val": _eval_scores(base_logits, y_true, valid, instance_ids, val_mask),
            "test": _eval_scores(base_logits, y_true, valid, instance_ids, test_mask),
        },
        "ensemble": {
