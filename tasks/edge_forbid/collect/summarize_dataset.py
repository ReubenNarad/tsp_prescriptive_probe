...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Summarize a merged edge-what-if dataset")
...
def main() -> None:
    ...
    forbid_time_wall = ds.get("forbid_time_wall")
    ...
    if not torch.is_tensor(valid_forbid) or valid_forbid.shape != (B, n):
    ...
    forbid_valid_frac = float(valid_forbid.float().mean().item()) if (B > 0 and n > 0) else 0.0
    ...
    summary: Dict[str, Any] = {
        ...
        "valid_forbid_frac": forbid_valid_frac,
        "meta": ds.get("meta", {}),
    }
    ...
    if torch.is_tensor(forbid_length):
        summary["forbid_length"] = _stats(_mask_to_np(forbid_length, pair_valid))
    if torch.is_tensor(forbid_time_wall):
        summary["forbid_time_wall"] = _stats(_mask_to_np(forbid_time_wall, pair_valid))
    ...
    print("[summary] valid_forbid_frac:", f"{summary['valid_forbid_frac']:.3f}")
