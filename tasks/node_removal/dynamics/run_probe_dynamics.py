...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Compute probe-vs-training-epoch curves for node-removal what-if.")
    p.add_argument("--run_dir", type=str, required=True, help="Policy run dir (or runs/<name>) containing checkpoints/")
    p.add_argument("--data_dir", type=str, required=True, help="Processed what-if dataset dir containing dataset.pt")
    p.add_argument(
        "--activation_key",
        type=str,
    ...
    p.add_argument("--device", type=str, default="cpu", help="Device for extraction + probe training (default: cpu).")
    ...
    p.add_argument("--epoch_step", type=int, default=50)
    p.add_argument("--min_epoch", type=int, default=None)
    ...
    p.add_argument("--keep_reps", action="store_true")
...
def _plot(summary_csv: Path, out_path: Path, *, n_nodes: int, smooth: int) -> None:
    ...
    ax.plot(x, top1, label="Top-1 (best node)", linewidth=2.0)
    ax.plot(x, top5, label="Top-5 contains best", linewidth=2.0)
    ax.plot(x, spearman, label="Spearman (node scores)", linewidth=2.0)

    chance_top1 = 1.0 / float(n_nodes)
    chance_top5 = min(5, int(n_nodes)) / float(n_nodes)
    ax.axhline(y=chance_top1, color="0.4", linestyle="--", linewidth=1.5, label=f"Chance top-1 (1/{n_nodes})")
    ax.axhline(y=chance_top5, color="0.4", linestyle=":", linewidth=1.5, label=f"Chance top-5 ({min(5,n_nodes)}/{n_nodes})")
    ...
    ax.set_title("Node-removal probe vs policy training epoch", fontsize=16)
...
def main() -> None:
    ...
    meta = ds.get("meta", {})
    n_nodes = int(meta.get("num_loc", 0) or 0)
    if n_nodes <= 0:
        locs = ds.get("locs")
        if torch.is_tensor(locs):
            n_nodes = int(locs.shape[-2])
    if n_nodes <= 0:
        raise ValueError("Could not infer num_loc from dataset meta or locs.")

    ckpts = _find_checkpoints(run_dir)
    avail_epochs = sorted(ckpts.keys())

    if args.epochs:
        epochs = _parse_epochs_arg(args.epochs)
    else:
    ...
    out_root = Path(args.out_root).expanduser().resolve() if args.out_root else (
        _repo_root() / "tasks" / "node_removal" / "tmp" / "dynamics" / f"{run_dir.name}__{data_dir.name}" / act_tag
    )
    out_root.mkdir(parents=True, exist_ok=True)

    summary_csv = out_root / "summary.csv"
    plot_path = out_root / "probe_dynamics.png"

    if args.plot_only:
        if not summary_csv.exists():
            raise FileNotFoundError(f"--plot_only set but missing {summary_csv}")
        if not args.no_plot:
            _plot(summary_csv, plot_path, n_nodes=int(n_nodes), smooth=int(args.smooth))
            print(f"[whatif-dynamics] wrote: {plot_path}")
        return

    extract_py = _repo_root() / "core" / "reps" / "extract_node_reps.py"
    ...
    for epoch in epochs:
        metrics_dir = out_root / f"probe_artifacts_epoch_{epoch}"
        metrics_path = metrics_dir / "metrics.json"

        if metrics_path.exists() and not args.overwrite:
            m = _read_json(metrics_path)
        else:
            reps_path = out_root / f"probe_reps_epoch_{epoch}.pt"
            if (not reps_path.exists()) or args.overwrite:
                ...
                print(f"[whatif-dynamics] epoch {epoch}: extract -> {reps_path}", flush=True)
                _run(cmd)
            ...
            print(f"[whatif-dynamics] epoch {epoch}: train -> {metrics_dir}", flush=True)
            _run(train_cmd)
        ...
        print(
            f"[whatif-dynamics] epoch {epoch}: top1={rows[-1]['top1_acc']:.3f} top5={rows[-1]['top5_acc']:.3f} "
            f"spearman={rows[-1]['spearman_mean']:.3f}",
            flush=True,
        )

    print(f"[whatif-dynamics] wrote: {summary_csv}")
    if not args.no_plot:
        _plot(summary_csv, plot_path, n_nodes=int(n_nodes), smooth=int(args.smooth))
        print(f"[whatif-dynamics] wrote: {plot_path}")
