...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Ensemble a probe model with a baseline model (both set-transformer regression)."
    )
    p.add_argument("--probe_reps_path", type=str, required=True)
    p.add_argument("--probe_model_path", type=str, required=True)
    p.add_argument("--baseline_reps_path", type=str, required=True)
    p.add_argument("--baseline_model_path", type=str, required=True)
    p.add_argument("--out_path", type=str, required=True)
    p.add_argument("--seed", type=int, default=0, help="Split seed (fixed sizes).")
...
def main() -> None:
    ...
    probe_scores, y_true, valid, instance_ids, B, n = _load_scores(
        reps_path=Path(args.probe_reps_path).expanduser().resolve(),
        model_path=Path(args.probe_model_path).expanduser().resolve(),
        device=device,
        batch_size_instances=int(args.batch_size_instances),
    )
    base_scores, _y2, _v2, instance_ids2, B2, n2 = _load_scores(
        reps_path=Path(args.baseline_reps_path).expanduser().resolve(),
        model_path=Path(args.baseline_model_path).expanduser().resolve(),
        device=device,
        batch_size_instances=int(args.batch_size_instances),
    )

    if B2 != B or n2 != n:
        raise ValueError(f"Baseline reps size mismatch: {(B2, n2)} vs {(B, n)}")
    if not np.array_equal(instance_ids, instance_ids2):
        raise ValueError("Baseline reps instance ordering differs from probe reps")
    ...
    if args.score_norm == "zscore":
        probe_scores = _zscore_by_instance(probe_scores, inst_ids, valid)
        base_scores = _zscore_by_instance(base_scores, inst_ids, valid)

    alpha_grid = _parse_alpha_grid(args.alpha_grid)
    val_curve = []
    for alpha in alpha_grid:
        mix = alpha * probe_scores + (1.0 - alpha) * base_scores
    ...
    mix = best_alpha * probe_scores + (1.0 - best_alpha) * base_scores

    out = {
        "config": {
            "probe_reps_path": str(Path(args.probe_reps_path).expanduser().resolve()),
            "probe_model_path": str(Path(args.probe_model_path).expanduser().resolve()),
            "baseline_reps_path": str(Path(args.baseline_reps_path).expanduser().resolve()),
            "baseline_model_path": str(Path(args.baseline_model_path).expanduser().resolve()),
            "seed": int(args.seed),
            "train_size": int(args.train_size),
            "val_size": int(args.val_size),
            "test_size": int(args.test_size),
        ...
        "probe_only": {
            "val": _eval_scores(probe_scores, y_true, valid, inst_ids, val_mask),
            "test": _eval_scores(probe_scores, y_true, valid, inst_ids, test_mask),
        },
        "baseline_only": {
            "val": _eval_scores(base_scores, y_true, valid, inst_ids, val_mask),
            "test": _eval_scores(base_scores, y_true, valid, inst_ids, test_mask),
        },
        "ensemble": {
