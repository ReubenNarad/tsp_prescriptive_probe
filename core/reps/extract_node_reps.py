...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--add_nn_dist",
        action="store_true",
        help="Also store per-node nearest-neighbor distance (from locs) as X_extra (one feature).",
    )
    return p
...
def main() -> None:
    ...
    with torch.inference_mode():
        ...
        while offset < B:
            ...
            if bool(args.add_nn_dist):
                # Nearest-neighbor Euclidean distance per node, computed from instance coordinates.
                # Shape: [current, n] -> flattened to [current*n, 1].
                dists = torch.cdist(locs_batch, locs_batch, p=2)  # on CPU
                eye = torch.eye(n, dtype=torch.bool).unsqueeze(0).expand(current, n, n)
                dists = dists.masked_fill(eye, float("inf"))
                nn = dists.min(dim=-1).values  # [current, n]
                X_extra_parts.append(nn.reshape(-1, 1).to("cpu", dtype=torch.float32))
    ...
    if X_extra_parts:
        out["X_extra"] = torch.cat(X_extra_parts, dim=0)
        out["extra_feature_names"] = ["nn_dist"]
